...
void FindReplaceDlg::updateDialog(void)
{
	...
	if (_findReplace == TRUE)
	{
		if (NLGetText(_hInst, _hParent, _T("Replace"), txtCaption, sizeof(txtCaption) / sizeof(TCHAR)) == FALSE)
			_tcscpy_s(txtCaption, _T("Replace"));
		...
		switch (process)
		{
			case COUNT:
			{
				if (NLGetText(_hInst, _hParent, _T("Tokens Found"), TEMP, 128)) {
					_stprintf_s(text, TEMP, cnt);
				} else {
					_stprintf_s(text, _T("%i tokens are found."), cnt);
				}
			...
			case REPLACE_ALL:
			{
				...
				if (NLGetText(_hInst, _hParent, _T("Tokens Replaced"), TEMP, 128)) {
					...
					_stprintf_s(text, _T("%i tokens are replaced.\n"), cnt);
				}

				if (cntError != 0)
				{
					if (NLGetText(_hInst, _hParent, _T("Tokens Skipped"), TEMP, 128)) {
						_stprintf_s(text, TEMP, text, cntError);
					} else {
						_stprintf_s(text, _T("%s%i tokens are skipped, because of alignment error.\n"), text, cntError);
					}
				}
